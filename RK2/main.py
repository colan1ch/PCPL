...
def make_one_to_many(books, libraries):
    return [
...
def make_many_to_many(books, libraries, books_to_libraries):
    many_to_many_temp = [
...
def task3(many_to_many):
    ...
    for book_title, book_author, pages_count, lib_name in sorted(many_to_many, key=itemgetter(3)):
        ans.append(f'Книга {book_title} автора {book_author} (кол-во страниц {pages_count}) находится в {lib_name}')
    return ans
...
def main():
    ...
    one_to_many = make_one_to_many(books, libraries)
    many_to_many = make_many_to_many(books, libraries, books_to_libraries)
